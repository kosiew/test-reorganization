...
// @author       Siew "@xizun"
// @match        https://schedule.happy.tools/*
...
;(function($){ //function to create private scope with $ parameter
    ...
    function checkHref() {
        ...
        if (href.startsWith('https://schedule.happy.tools/landing')) {
            clearInterval(intervalFunc);
        } else {
            dlog(`href ${href} vs ${previousHref}`);
            if (previousHref != undefined && href != previousHref && (previousHref == SCHEDULE_HREF 
                || href == SCHEDULE_HREF)) {
                dlog('reloading');
                location.reload();
            }
